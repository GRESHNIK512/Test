...
#pragma warning restore CS0618 // Type or member is obsolete
        ...
        //SendByConnection
        public static void SendToConn<T>(T message, NetworkConnectionToClient conn, int channelId = Channels.Reliable)
           where T : struct, NetworkMessage
        {
            if (!active)
            {
                Debug.LogWarning("Can not send using NetworkServer.SendToConn<T>(T msg) because NetworkServer is not active");
                return;
            }

            // Debug.Log($"Server.SendToConn {typeof(T)}");
            using (NetworkWriterPooled writer = NetworkWriterPool.Get())
            {
                // pack message only once
                NetworkMessages.Pack(message, writer);
                ...
                if (writer.Position > max)
                {
                    Debug.LogError($"NetworkServer.SendToConn: message of type {typeof(T)} with a size of {writer.Position} bytes is larger than the max allowed message size in one batch: {max}.\nThe message was dropped, please make it smaller.");
                    return;
                }
